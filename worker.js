  ...
  input_schema: {
    ...
    properties: {
      existing_customers: {
        ...
        required: ['customer_sentiments', 'topic_tags'],
      },
      prospecting: {
        type: 'object',
        properties: {
...
function buildStaticInstructions() {
  return [
    ...
    'Geef als eerste veld "customer_sentiments" terug — geen samenvatting, maar een volledige en uitputtende lijst: exact één entry per genummerd opmerking-id bij "BESTAANDE KLANTEN" (elk id begint met "R", bv. "R1"), in dezelfde volgorde, zonder er één over te slaan en zonder ids te verzinnen. Doe dit VOORDAT je aan "topic_tags" begint — bij een lange opmerkingenlijst kan het antwoord de lengtelimiet raken, en deze lijst bepaalt rechtstreeks de score, dus mag nooit ontbreken.',
    ...
    '- "negative": de klant zelf uit ontevredenheid of keert zich (deels) af — een aanhoudende of onopgeloste klacht, een prijs- of voorwaardenbezwaar, verlies van vertrouwen, omzet die naar een concurrent gaat, of twijfel om verder samen te werken (bv. "te duur t.o.v. concurrent, koopt daar nu", "vertrouwen kwijt na herhaalde klachten", "ontevreden over service").',
    ...
    'Let op bij "no_opinion": een opmerking waarin een concurrent (een deel van) de omzet in deze categorie binnenhaalt, of waarin de klant deze categorie elders koopt, is nooit "no_opinion" — daar zit wel degelijk informatie in. Kies "negative" wanneer er een bezwaar tegen Winsol bij staat en anders "neutral", en leg de concurrent in beide gevallen vast in "competitor".',
    ...
    'BELANGRIJK — een technische vaststelling is op zichzelf GEEN negatief oordeel van de klant. Interventie- en servicerapporten beschrijven wat een technieker vaststelt of oplost (bv. "Problème sur 2 Solarfix - 1 fiche débranchée", "windsensor defect, te vervangen", "motor vervangen onder garantie", "volet Solar que le client n arrive pas à résoudre"). Zolang de klant zich daarbij niet ontevreden uit, is dat "neutral". Pas wanneer de tekst zelf de ontevredenheid, het herhaalkarakter of het commerciële gevolg benoemt (bv. "al de derde keer", "klant is het beu", "klant dreigt over te stappen", "blijft zonder oplossing ondanks meldingen") wordt het "negative".',
    'BELANGRIJK — beoordeel het sentiment uitsluitend op wat er in de opmerking over DEZE categorie staat. De algemene toon van een bezoekverslag telt niet mee: een zwaar negatief verslag over schrijnwerk maakt de categorie "luifels" niet negatief wanneer luifels daarin enkel neutraal of positief voorkomen. Staat er over deze categorie zelf niets negatiefs, dan is het sentiment nooit "negative", hoe somber de rest van het verslag ook is. Dit verandert NIETS aan de meldplicht hierboven: je geeft nog steeds een entry voor elk id. Gaat een opmerking niet over deze categorie, dan is het antwoord "no_opinion" voor dat id — nooit een weggelaten id en nooit een lege lijst.',
    'BELANGRIJK — de loutere vaststelling dat een concurrent deze categorie vandaag levert, zonder uitgesproken bezwaar tegen Winsol, is "neutral" (de tag "concurrentie" legt dat feit al vast). Geeft de klant er wél een reden of bezwaar bij (prijs, service, voorwaarden, acties, vertrouwen), dan is het "negative". Een genoemde reden waarom de klant deze categorie NIET bij Winsol koopt telt altijd als een dergelijk bezwaar, ook zonder klachttoon en ook wanneer ze nuchter-feitelijk geformuleerd is (bv. "prijsverschil van 15% om gelijk te zitten", "koopt de rolluiken al jaren bij X", "krijgt daar grotere kortingen ongeacht het zakencijfer"). Dat is "negative", niet "neutral".',
    'Ken je om één van deze redenen "neutral" toe, blijf het onderwerp dan wél taggen in "topic_tags" (bv. een technische melding met sentiment "negative" t.o.v. dat onderwerp). De vaststelling gaat dus niet verloren — ze telt enkel niet mee als een negatief oordeel van de klant zelf.',
    ...
    'ABSOLUTE REGEL: staan er ids in de opdracht, dan bevat "customer_sentiments" exact evenveel entries als er ids zijn. Een leeg "customer_sentiments" is in dat geval altijd fout, ook wanneer geen enkele opmerking over de categorie blijkt te gaan — geef dan voor elk id "no_opinion" en laat "topic_tags" leeg.',
    '',
